// DO NOT USE THIS. this is not audited in the scope of mainnet launch.
// This code is an early example and for reference only
// For oft, go to layerzero_apps::oft
module layerzero::oft {
    ...
    const EOFT_INVALID_SEND_PAYLOAD: u64 = 0x03;
    ...
    struct EventStore has key {
        ...
        invalid_receive_events: EventHandle<InvalidReceiveEvent>,
    ...
    struct InvalidReceiveEvent has drop, store {
        src_chain_id: u64,
        receiver: vector<u8>,
        amount: u64,
    }
    ...
    fun init<OFT>(account: &signer, proxy: bool, shared_decimals: u8): UaCapability<OFT> {
        ...
        move_to(account, EventStore {
            ...
            invalid_receive_events: account::new_event_handle<InvalidReceiveEvent>(account),
    ...
    public fun lz_receive<OFT>(
        ...
        let (receiver_bytes, amount_sd) = decode_send_payload(&payload);
        let amount = sd2ld(amount_sd, global_store.ld2sd_rate);

        // if invalid address, transfer coin to oft owner instead
        let is_valid = vector::length(&receiver_bytes) == 32;
        let receiver =  if (is_valid) {
            from_bcs::to_address(receiver_bytes)
        } else {
            oft_address
        };
        ...
        if (is_valid) {
            ...
            event::emit_event<InvalidReceiveEvent>(
                &mut event_store.invalid_receive_events,
                InvalidReceiveEvent {
                    src_chain_id,
    ...
    public fun quote_fee<OFT>(
        ...
        let payload = encode_send_payload(dst_receiver, amount);
        endpoint::quote_fee(
            type_address<OFT>(),
            dst_chain_id,
    ...
    // internal functions
    //
    fun unlock_or_mint_coin<OFT>(receiver: address, amount: u64): bool acquires CoinStore, GlobalStore, CoinCapabilities {
        let oft_address = type_address<OFT>();
        let global_store = borrow_global<GlobalStore<OFT>>(oft_address);
        let coin_store = borrow_global_mut<CoinStore<OFT>>(oft_address);
    ...
    fun encode_send_payload(dst_receiver: vector<u8>, amount: u64): vector<u8> {
    ...
    // the size of receiver address may not be 32 bytes if user send with a wrong address from src chain
    fun decode_send_payload(payload: &vector<u8>): (vector<u8>, u64) {
        let packet_type = serde::deserialize_u8(&vector_slice(payload, 0, 1));
        let payload_size = vector::length(payload);
        assert!(packet_type == PT_SEND && payload_size >= 9, error::invalid_argument(EOFT_INVALID_SEND_PAYLOAD));

        let receiver = vector_slice(payload, 1, payload_size - 8);
        let amount_sd = serde::deserialize_u64(&vector_slice(payload, payload_size - 8, payload_size));
        (receiver, amount_sd)
    }
    ...
    fun test_encode_and_decode_send_payload() {
        ...
        assert!(from_bcs::to_address(actual_receiver) == receiver, 0);
        assert!(amount == actual_amount, 0);
    }

    #[test]
    fun test_decode_send_payload_wit_invalid_address() {
        let receiver = x"123456";
        let amount = 100;
        let payload = encode_send_payload(receiver, amount);
        assert!(payload == x"001234560000000000000064", 0);
    ...
    fun test_decode_send_payload_invalid_length() {
        decode_send_payload(&x"123456");
    }

    #[test]
    #[expected_failure(abort_code = 0x10003, location = Self)]
    fun test_decode_send_payload_invalid_type() {
        // packet type is 0x01
        decode_send_payload(&x"0100000000000000000000000000000000000000000000000000000000001234560000000000000064");
    }

    #[test_only]
    use layerzero::test_helpers;
    #[test_only]
    use test::oft_test::MoonOFT;
